...
# Function to normalize audio duration
def normalize_duration(y, sr, target_duration):
    current_duration = librosa.get_duration(y=y, sr=sr)
    target_length = int(target_duration * sr)

    if current_duration > target_duration:
        # Time-stretch to shrink to target duration
        rate = current_duration / target_duration
        y = librosa.effects.time_stretch(y, rate=rate)
    elif current_duration < target_duration:
        # Pad with zeros (silence) to match length
        padding = target_length - len(y)
        y = np.pad(y, (0, padding), mode='constant')
    
    return y

# Function to compute MFCCs with silence trimming, deltas, and duration normalization
...
for dataset in datasets:
    ...
    for i in range(1, num_pairs + 1):
        ...
        # Load MFCCs (trim, normalize duration, add deltas)
